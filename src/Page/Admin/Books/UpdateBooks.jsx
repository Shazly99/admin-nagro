...
const UpdateBooks = () => {
  ...
  let { id } = useParams()
  const urlUpdateData = `${process.env.REACT_APP_API_URL}/books/${id}/update`;
  const urlFetchData = `${process.env.REACT_APP_API_URL}/books/${id}`;
  const [loading, setLoading] = useState(false);
  const [Data, setData] = useState(false);
  const toast = useRef(null);
  let navigate = useNavigate()
  const items = [
    { label: <Link to={'/books'}>  {isLang === "en" ? "books " : 'الاخبار و المقالات'} </Link> },
    { label: <Link to={`/books/edit/${id}`} className='bg-orange-100 p-2 border-round'>  {isLang === "en" ? "Update books " : 'تعديل بيانات مقاله'}  </Link> }
  ];
  ...
  const [selectMultiImageFile, setSelectMultiImageFile] = useState(null);

  const handleMultiImageSelectFiles = (event) => {
    setSelectMultiImageFile(event.target.files[0])
  }
  ...
  useEffect(() => {
    formik.setFieldValue('file', selectMultiImageFile);
      ...
      <form onSubmit={formik.handleSubmit} className='mt-6 w-11  m-auto my-8'>
          ...
          <div className='lg:col-6 md:col-12 sm:col-12 mt-2 flex justify-content-center align-items-center flex-column gap-4'>
            <div className="mt-3 flex justify-content-center m-auto" style={{ position: 'relative', marginBottom: '30px', width: 'fit-content' }}>
              {selectMultiImageFile ? (
                <iframe
                  title={`PDF `}
                  src={URL.createObjectURL(selectMultiImageFile)}
                  style={{ objectFit: 'contain' }}
                  className='w-full h-15rem border-round-xl'
                />
              ) : (
                <iframe
                  title={`PDF `}
              ...
              <div style={{ position: 'absolute', right: '-5px', bottom: '0px' }}>
                <input
                  ...
                  id="file-input_pdf"
                  accept=".pdf"
                  onChange={handleMultiImageSelectFiles}
                  style={{ display: 'none' }}
                />
                <label htmlFor="file-input_pdf" className="btn__porfile" style={{ pointerEvents: 'all' }}>
                  <i className='pi pi-file-pdf color-wight'></i>
                </label>
              </div>
            </div>
            <label htmlFor="file-input_pdf" className="font-bold text-xl block mb-2">
              {isLang === 'en' ? 'Book PDF' : 'نسخة من الكتاب'}
            </label>
          </div>
        </div>
        ...
        <div className="grid  mt-5  ">
          <div className="lg:col-6 md:col-12 sm:col-12 mt-2 input__Col ">
            ...
            <InputText name='short_title_en' value={formik.values.short_title_en} id="short_title_en" type="text" className="w-full  p-inputtext-sm" onChange={formik.handleChange}
              onBlur={formik.handleBlur} />
          </div>
          <div className="lg:col-6 md:col-12 sm:col-12   mt-2 input__Col" >
            <label htmlFor="short_title_ar" className="font-bold block mb-2">   {isLang === "en" ? "Short title ar" : 'العنوان قصير بالعربي '} </label>
            <InputText name='short_title_ar' value={formik.values.short_title_ar} id="short_title_ar" type="text" className="w-full  p-inputtext-sm" dir='rtl' onChange={formik.handleChange}
              onBlur={formik.handleBlur} />
          </div>
        </div>
        ...
        <div className="grid  mt-2   ">
          <div className="lg:col-6 md:col-12 sm:col-12 mt-2 input__Col ">
            ...
            <InputTextarea
              rows={5} name='short_description_en' value={formik.values.short_description_en} id="short_description_en" type="text" className="w-full  p-inputtext-sm" onChange={formik.handleChange}
              onBlur={formik.handleBlur} />
          </div>
          <div className="lg:col-6 md:col-12 sm:col-12   mt-2 input__Col">
            <label htmlFor="short_description_ar" className="font-bold block mb-2">  {isLang === "en" ? "Short Description ar" : 'وصف قصير بالعربي'}  </label>
            <InputTextarea
              rows={5} name='short_description_ar' value={formik.values.short_description_ar} id="short_description_ar" type="text" className="w-full  p-inputtext-sm" dir='rtl'
