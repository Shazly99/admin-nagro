...
const UpdateAbout = () => {
    ...
    let { id } = useParams()
    const urlUpdateData = `${process.env.REACT_APP_API_URL}/about-us/${id}/update`;
    const urlFetchData = `${process.env.REACT_APP_API_URL}/about-us/${id}`;
    const [loading, setLoading] = useState(false);
    const [Data, setData] = useState(false);
    const toast = useRef(null);
    let navigate = useNavigate()
    const items = [
        { label: <Link to={'/about'}>  {isLang === "en" ? "Abouts  " : ' من نحن   '} </Link> },
        { label: <Link to={`/about/edit/${id}`} className='bg-orange-100 p-2 border-round'>  {isLang === "en" ? "Update Abouts  " : 'تعديل علي نبذة عنا'}  </Link> }
    ];
