...
const UpdateWhyUs = () => {
  ...
  let { id } = useParams()
  const urlUpdateData = `${process.env.REACT_APP_API_URL}/why-us/${id}/update`;
  const urlFetchData = `${process.env.REACT_APP_API_URL}/why-us/${id}`;
  const [loading, setLoading] = useState(false);
  const [Data, setData] = useState(false);
  const toast = useRef(null);
  let navigate = useNavigate()
  const items = [
    { label: <Link to={'/why-us'}>  {isLang === "en" ? "Why us " : ' من نحن   '} </Link> },
    { label: <Link to={`/why-us/edit/${id}`} className='bg-orange-100 p-2 border-round'>  {isLang === "en" ? "Update Why us " : 'تحديث لماذا نحن'}  </Link> }
  ];
