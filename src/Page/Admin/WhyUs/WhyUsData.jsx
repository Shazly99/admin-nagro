...
const WhyUsData = () => {
    ...
    const leftToolbarTemplate = () => {
        return (
            <div className="btn_gapAr_header flex flex-wrap gap-2">
                <Link to="/why-us/add">
                    <Button label={isLang === "en" ? "New why us" : '  أضف لماذا نحن   '} icon="pi pi-plus" size='small' />
                </Link>
            </div>
        );
    };
    ...
    const imageBodyTemplate = (rowData) => {
        return <div className="flex justify-content-center align-items-center">
            <img src={rowData.image} alt={'image why us'} className="shadow-2 border-round" style={{ width: '150px' }} />
        </div>
    };

    // handel icons in tabel
    const iconBodyTemplate = (rowData) => {
        return <div className="flex justify-content-center align-items-center">
            <img src={rowData.icon} alt={'icon why us'} className="shadow-2" style={{ width: '100px', height: '100px', borderRadius: '50%', objectFit: 'contain' }} />
        </div>
    };
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <div className="flex gap-2">
                <Link to={`/why-us/edit/${rowData.id}`}>
    ...
    const deleteWhy = async () => {
        ...
        await axios.delete(`${process.env.REACT_APP_API_URL}/why-us/${IdDeleteWhy}`, {
            headers: {
                'Content-Type': 'multipart/form-data',
                // 'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('tokenNagro'),
        ...
        <div className='mb-8'>
            ...
            <div className="card ">
                ...
                <DataTable
                    ...
                    <Column field="icon" header={isLang === "en" ? "icon" : 'رمز'} body={iconBodyTemplate}></Column>
