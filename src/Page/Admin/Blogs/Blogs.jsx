...
const Blogs = () => {
    ...
    const leftToolbarTemplate = () => {
      return (
        <div className="btn_gapAr_header flex flex-wrap gap-2">
          <Link to="/blogs/add">
            <Button label={isLang === "en" ? "New blogs" : 'إضـــافة مقاله'} icon="pi pi-plus" size='small' />
          </Link>
        </div>
      );
    };
    ...
    const imageBodyTemplate = (rowData) => {
      return <div className="flex justify-content-center align-items-center">
        <img src={rowData.image} alt={'image blogs'}  className="shadow-2" style={{ width: '100px', height: '100px', borderRadius: '50%', objectFit: 'contain' }}  />
      </div>
    };
    ...
    const deleteWhy = async () => {
      ...
      await axios.delete(`${process.env.REACT_APP_API_URL}/blogs/${IdDeleteWhy}`, {
        headers: {
          'Content-Type': 'multipart/form-data',
          // 'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + localStorage.getItem('tokenNagro'),
